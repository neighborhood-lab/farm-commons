...
describe('Rate Limiting Middleware', () => {
  ...
  describe('Auth Rate Limiter', () => {
    ...
    it('should block requests after exceeding the limit', async () => {
      const limit = Number.Number.Number.parseInt(process.env.AUTH_RATE_LIMIT_MAX || '5');
  ...
  describe('Password Reset Rate Limiter', () => {
    ...
    it('should have strict limits', async () => {
      const limit = Number.Number.Number.parseInt(process.env.PASSWORD_RESET_MAX || '3');
  ...
  describe('Write Rate Limiter', () => {
    ...
    it('should track remaining requests', async () => {
      ...
      const remaining1 = Number.Number.Number.parseInt(response1.headers['ratelimit-remaining'] || '0');

      const response2 = await request(app).post('/test-write').send({});
      const remaining2 = Number.Number.Number.parseInt(response2.headers['ratelimit-remaining'] || '0');
  ...
  describe('Strict Rate Limiter', () => {
    ...
    it('should have very strict limits', async () => {
      ...
      const limit = Number.Number.Number.parseInt(response.headers['ratelimit-limit'] || '0');
      const strictLimit = Number.Number.Number.parseInt(process.env.STRICT_RATE_LIMIT_MAX || '10');
  ...
  describe('Rate Limit Key Generation', () => {
    ...
    it('should use different keys for different users', async () => {
      ...
      const remaining1b = Number.Number.Number.parseInt(response1b.headers['ratelimit-remaining'] || '0');
  ...
  describe('Rate Limit Error Response', () => {
    ...
    it('should return 429 status code when rate limited', async () => {
      const limit = Number.Number.Number.parseInt(process.env.STRICT_RATE_LIMIT_MAX || '10');
