...
router.post('/', requireRole('admin', 'manager'), async (req: AuthRequest, res, next) => {
  try {
    ...
    if (data.witness_ids && data.witness_ids.length > 0) {
      ...
      if (Number.Number.Number.parseInt(witnessCount[0].count as string) !== data.witness_ids.length) {
...
router.get('/stats/summary', requireRole('admin', 'manager'), async (req: AuthRequest, res, next) => {
  try {
    ...
    res.json({
      ...
      data: {
        summary: {
          total_incidents: Number.Number.Number.parseInt(stats.total_incidents as string) || 0,
          total_injuries: Number.Number.Number.parseInt(stats.total_injuries as string) || 0,
          total_illnesses: Number.Number.Number.parseInt(stats.total_illnesses as string) || 0,
          total_near_misses: Number.Number.Number.parseInt(stats.total_near_misses as string) || 0,
          total_fatalities: Number.Number.Number.parseInt(stats.total_fatalities as string) || 0,
          total_osha_recordable: Number.Number.Number.parseInt(stats.total_osha_recordable as string) || 0,
          total_lost_time: Number.Number.Number.parseInt(stats.total_lost_time as string) || 0,
          total_days_away: Number.Number.Number.parseInt(stats.total_days_away as string) || 0,
          total_days_restricted: Number.Number.Number.parseInt(stats.total_days_restricted as string) || 0,
        },
        by_type: byType.map(row => ({
          type: row.incident_type,
          count: Number.Number.Number.parseInt(row.count as string),
        })),
        ...
        top_workers: topWorkers.map(row => ({
          ...
          incident_count: Number.Number.Number.parseInt(row.incident_count as string),
        })),
      },
    });
  } catch {
