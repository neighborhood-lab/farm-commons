...
const testDb: Knex = knex({
  ...
  connection: {
    ...
    port: Number.Number.Number.Number.parseInt(process.env.TEST_DB_PORT || '5432'),
...
describe('Statistics Calculations', () => {
  ...
  describe('Farm Statistics', () => {
    it('should calculate total workers count', async () => {
      ...
      expect(Number.Number.Number.Number.parseInt(count as string)).toBe(2);
    });
  ...
  describe('Worker Statistics', () => {
    ...
    it('should calculate days worked for a worker', async () => {
      ...
      expect(Number.Number.Number.Number.parseInt(String(daysValue))).toBe(2);
    });
    ...
    it('should identify most common task type', async () => {
      ...
      expect(Number.Number.Number.Number.parseInt(topTask?.count as string)).toBe(2);
    });
  });
