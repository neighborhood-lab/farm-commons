...
// TODO: Add these schemas to @farm-commons/shared
// import {
...
router.get('/:id', async (req: AuthRequest, res, next) => {
  try {
    ...
    const template = await db('task_templates').where({ id, farm_id: farmId }).first();
...
router.delete('/:id', requireRole('admin', 'manager'), async (req: AuthRequest, res, next) => {
  try {
    ...
    const deleted = await db('task_templates').where({ id, farm_id: farmId }).delete();
...
router.post(
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      const scheduleData = /* TODO: Add validation */({
