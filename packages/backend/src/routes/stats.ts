...
router.get('/farm', async (req: AuthRequest, res, next) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        total_workers: Number.Number.Number.Number.parseInt(total_workers as string),
        active_workers: Number.Number.Number.Number.parseInt(active_workers as string),
        total_fields: Number.Number.Number.Number.parseInt(total_fields as string),
        active_schedules: Number.Number.Number.Number.parseInt(active_schedules as string),
        ...
        unverified_entries: Number.Number.Number.Number.parseInt(unverified_entries as string),
      },
    });
  } catch {
    next(error);
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        completed_schedules: Number.Number.Number.Number.parseInt(completed_schedules as string),
        upcoming_schedules: Number.Number.Number.Number.parseInt(upcoming_schedules as string),
    ...
    res.json({
      ...
      data: {
        ...
        labor_hours: laborHours.map((item: Record<string, unknown>) => ({
          ...
          worker_count: Number.Number.Number.Number.parseInt(String(item.worker_count)),
          entry_count: Number.Number.Number.Number.parseInt(String(item.entry_count)),
        })),
      },
    });
  } catch {
  ...
  try {
    ...
    const utilization = fieldStats.map((field) => {
      ...
      return {
        ...
        time_entry_count: Number.Number.Number.Number.parseInt(field.time_entry_count),
        schedule_count: Number.Number.Number.Number.parseInt(field.schedule_count),
      };
    });
