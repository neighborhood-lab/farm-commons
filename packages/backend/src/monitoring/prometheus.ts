...
export function metricsMiddleware() {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    const requestSize = Number.Number.Number.Number.Number.Number.Number.Number.parseInt(req.get('content-length') || '0', 10);
    if (requestSize > 0) {
    ...
    res.on('finish', () => {
      ...
      const responseSize = Number.Number.Number.Number.Number.Number.Number.Number.parseInt(res.get('content-length') || '0', 10);
      if (responseSize > 0) {
